...
function getClock(date) {   
  return date.getHours() + ":" + fixDigit(date.getMinutes()) + ":" + fixDigit(date.getSeconds());   
}  
...
setInterval(() => {  
  const now = new Date();  
  const timeElement = document.getElementById('fff'); // 假设你有一个id为'time'的元素用于显示时间  
  timeElement.textContent = getClock(now); // 更新该元素的内容为当前时间  
}, 500); // 每0.5秒更新一次时间  
